...
-- Table: avalon.hex_path_path_lookup
-- lookup table for hex_paths and their associated paths.

DROP TABLE IF EXISTS avalon.hex_path_path_lookup CASCADE;

CREATE TABLE avalon.hex_path_path_lookup
(
  hex_path_id smallint NOT NULL,
  vector3_id smallint NOT NULL,
  CONSTRAINT hex_path_path_lookup_hex_path_id_vector3_id_pk PRIMARY KEY (hex_path_id, vector3_id),
  CONSTRAINT hex_path_path_lookup_hex_path_id_fk FOREIGN KEY (hex_path_id)
    REFERENCES avalon.hex_path (hex_path_id) MATCH SIMPLE
    ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT hex_path_path_lookup_vector3_id_fk FOREIGN KEY (vector3_id)
    REFERENCES avalon.vector3 (vector3_id) MATCH SIMPLE
    ON UPDATE NO ACTION ON DELETE NO ACTION
);
