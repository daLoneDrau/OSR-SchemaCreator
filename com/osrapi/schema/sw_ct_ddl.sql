...
-- Table: sw_ct.life_event
-- TODO add table description

DROP TABLE IF EXISTS sw_ct.life_event CASCADE;

CREATE SEQUENCE sw_ct.life_event_id_seq MINVALUE 0;

CREATE TABLE sw_ct.life_event
(
  life_event_id smallint DEFAULT nextval('sw_ct.life_event_id_seq') NOT NULL,
  description text NOT NULL,
...
-- Table: sw_ct.life_event_modifiers_lookup
-- lookup table for life_events and their associated modifierss.

DROP TABLE IF EXISTS sw_ct.life_event_modifiers_lookup CASCADE;

CREATE TABLE sw_ct.life_event_modifiers_lookup
(
  ...
  CONSTRAINT life_event_modifiers_lookup_life_event_id_fk FOREIGN KEY (life_event_id)
    REFERENCES sw_ct.life_event (life_event_id) MATCH SIMPLE
    ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT life_event_modifiers_lookup_equipment_item_modifier_id_fk FOREIGN KEY (equipment_item_modifier_id)
    REFERENCES sw_ct.equipment_item_modifier (equipment_item_modifier_id) MATCH SIMPLE
    ON UPDATE NO ACTION ON DELETE NO ACTION
);
...
-- Table: sw_ct.io_pc_data_life_events_lookup
-- lookup table for io_pc_datas and their associated life_eventss.

DROP TABLE IF EXISTS sw_ct.io_pc_data_life_events_lookup CASCADE;

CREATE TABLE sw_ct.io_pc_data_life_events_lookup
(
  io_pc_data_id smallint NOT NULL,
  life_event_id smallint NOT NULL,
