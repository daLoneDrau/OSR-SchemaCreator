...
public class TextProcessor {
	...
	public static TextProcessor getInstance() {
		if (TextProcessor.instance == null) {
		...
		try {
			if (string.length() > width) {
				...
				for (int i = 0, len = split.length; i < len; i++) {
					...
					sb.append(split[i]);
					for (int j = width - left - split[i].length(); j > 0; j--) {
						sb.append(' ');
					}
					if (i + 1 < len) {
						sb.append('\n');
				...
				sb.append(string);
				for (int j = width - left - string.length(); j > 0; j--) {
					sb.append(' ');
				}
			}
		} catch (PooledException e) {
	 ...
	 * @throws RPGException if an error occurs
	 */
	public String processText(final String generalText,
	 ...
	 * @throws RPGException if an error occurs
	 */
	public String processText(final String generalText, final String text)
			throws RPGException {
		return processText(new String[] { generalText },
				(boolean[]) null, text);
	}
	...
	public String processText(final String[] generalText,
		...
		/*
		 * if (tokenized.indexOf("<ioName>") > -1) { tokenized =
		 * tokenized.replaceAll("<ioName>", new String(playerIO
		 * .getPCData().getName())); } if (tokenized.indexOf("<ioNameApos>") >
		 * -1) { PooledStringBuilder sb =
		 * StringBuilderPool.getInstance().getStringBuilder(); String name = new
		 * String(playerIO.getPCData().getName()); try { sb.append(name); if
		 * (name.toLowerCase().charAt(sb.length() - 1) == 's' ||
		 * name.toLowerCase().charAt(sb.length() - 1) == 'x' ||
		 * name.toLowerCase().charAt(sb.length() - 1) == 'z') { sb.append('\'');
		 * } else { sb.append("'s"); } } catch (PooledException e) { throw new
		 * RPGException(ErrorMessage.INTERNAL_ERROR, e); } tokenized =
		 * tokenized.replaceAll("<ioNameApos>", sb.toString());
		 * sb.returnToPool(); sb = null; } if (tokenized.indexOf("<ioGenObj>") >
		 * -1) { String genObj = new String(
		 * Gender.GENDER_OBJECTIVE[playerIO.getPCData() .getGender()]);
		 * tokenized = tokenized.replaceAll("<ioGenObj>", genObj); genObj =
		 * null; } if (tokenized.indexOf("<ioGenPoss>") > -1) { String genPoss =
		 * new String( Gender.GENDER_POSSESSIVE[playerIO.getPCData()
		 * .getGender()]); tokenized = tokenized.replaceAll("<ioGenPoss>",
		 * genPoss); genPoss = null; } if (tokenized.indexOf("<ioGenPossObj>") >
		 * -1) { String genPossObj = new String(
		 * Gender.GENDER_POSSESSIVE_OBJECTIVE[playerIO .getPCData()
		 * .getGender()]); tokenized = tokenized.replaceAll("<ioGenPossObj>",
		 * genPossObj); genPossObj = null; } if (tokenized.indexOf("<ioGenPro>")
		 * > -1) { String genPro = new String(
		 * Gender.GENDER_PRONOUN[playerIO.getPCData() .getGender()]); tokenized
		 * = tokenized.replaceAll("<ioGenPossObj>", genPro); genPro = null; } if
		 * (tokenized.indexOf("<tioName>") > -1) { if
		 * (targetIO.hasIOFlag(Io.IO_03_NPC)) { tokenized =
		 * tokenized.replaceAll("<tioName>", new String(targetIO
		 * .getNPCData().getName())); } else if
		 * (targetIO.hasIOFlag(Io.IO_14_PARTY)) { tokenized =
		 * tokenized.replaceAll("<tioName>", new String( ((BpIoGroup)
		 * targetIO).getName())); } else if (targetIO.hasIOFlag(Io.IO_02_ITEM))
		 * { tokenized = tokenized.replaceAll("<tioName>", new String(targetIO
		 * .getItemData().getItemName())); } } if
		 * (tokenized.indexOf("<tioNameApos>") > -1) { PooledStringBuilder sb =
		 * StringBuilderPool.getInstance().getStringBuilder(); try { String name
		 * = ""; if (targetIO.hasIOFlag(Io.IO_03_NPC)) { name = new
		 * String(targetIO.getNPCData().getName()); } else if
		 * (targetIO.hasIOFlag(Io.IO_14_PARTY)) { name = new String(((BpIoGroup)
		 * targetIO).getName()); } else if (targetIO.hasIOFlag(Io.IO_02_ITEM)) {
		 * name = new String(targetIO.getItemData().getItemName()); }
		 * sb.append(name); if (name.toLowerCase().charAt(sb.length() - 1) ==
		 * 's' || name.toLowerCase().charAt(sb.length() - 1) == 'x' ||
		 * name.toLowerCase().charAt(sb.length() - 1) == 'z') { sb.append('\'');
		 * } else { sb.append("'s"); } tokenized = tokenized.replaceAll(
		 * "<tioNameApos>", sb.toString()); } catch (PooledException e) { throw
		 * new RPGException(ErrorMessage.INTERNAL_ERROR, e); }
		 * sb.returnToPool(); sb = null; } if
		 * (tokenized.indexOf("<tioGroupApos>") > -1) { PooledStringBuilder sb =
		 * StringBuilderPool.getInstance().getStringBuilder(); String tName =
		 * ((BpIoGroup) targetIO).getName(); try { sb.append(tName); if
		 * (tName.toLowerCase().charAt(sb.length() - 1) == 's' ||
		 * tName.toLowerCase().charAt(sb.length() - 1) == 'x' ||
		 * tName.toLowerCase().charAt(sb.length() - 1) == 'z') {
		 * sb.append('\''); } else { sb.append("'s"); } } catch (PooledException
		 * e) { throw new RPGException(ErrorMessage.INTERNAL_ERROR, e); }
		 * tokenized = tokenized.replaceAll("<tioGroupApos>", sb.toString());
		 * sb.returnToPool(); sb = null; } if (tokenized.indexOf("<tioGroup>") >
		 * -1) { tokenized = tokenized.replaceAll( "<tioGroup>", ((BpIoGroup)
		 * targetIO).getName()); }
		 */
	 ...
	 * @throws RPGException if an error occurs
	 */
	public final String processText(final String[] generalText,
			final String text) throws RPGException {
	 ...
	 * @throws RPGException if an error occurs
	 */
	public String processText(final String[] patterns,
		...
		String tokenized = text;
		tokenized = tokenized.replaceAll("\t", TextProcessor.TAB);
		if (patterns.length != generalText.length) {
			throw new RPGException(ErrorMessage.BAD_PARAMETERS,
					"The number of patterns and replacement strings "
							+ "do not match up.");
		}
